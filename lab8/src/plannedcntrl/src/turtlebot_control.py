...
def controller(waypoint):
  ...
  Kp = np.diag([2.0, 1.6]) # TODO: You may need to tune these values for your turtlebot
  Kd = np.diag([-0.4, 0.4]) # TODO: You may need to tune these values for your turtlebot
  Ki = np.diag([0, 0])

  prev_time = rospy.get_time() # : initialize your time, what rospy function would be helpful here?
  ...
  while not rospy.is_shutdown():
    ...
    try:
      ...
      # PROBABLY target_frame = camera_link, source = odom 
      ...
      waypoint_trans = PoseStamped() # : initialize a PoseStamped
      ...
      print("ABS ERRORS")
      print(np.abs(error[0]))
      print(np.abs(error[1]))
      threshold = 0.05
      if np.abs(error[0]) <= threshold and np.abs(error[1]) <= threshold: #TODO: what is our stopping condition/how do we know to go to the next waypoint?
...
def planning_callback(msg):
  try:
    ...
    coordtup = (msg.x, msg.y)
    trajectory = plan_curved_trajectory(coordtup) # TODO: What is the tuple input to this function?
