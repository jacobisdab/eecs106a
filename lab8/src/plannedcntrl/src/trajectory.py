...
def bezier_curve(p0, p1, p2, p3, t):
    ...
    return (1 - t) ** 3 * p0 + 3 * (1 - t) ** 2 * t * p1 + 3 * (1 - t) * t ** 2 * p2 + t ** 3 * p3 ## TODO
...
def plan_curved_trajectory(target_position):
    ...
    v = np.array([target_position[0], target_position[1], 0, 1])
    b = np.array([
        [np.cos(yaw), -np.sin(yaw), 0, x1],
        [np.sin(yaw), np.cos(yaw), 0, y1],
        [0, 0, 1, 0],
        [0, 0, 0, 1]
    ]) @ v # previously np.array([x1, y1, 1])
    x2 = b[0] ## TODO: how would you get x2 from our target position? remember this is relative to x1 
    y2 = b[1] ## TODO: how would you get x2 from our target position? remember this is relative to x1 
